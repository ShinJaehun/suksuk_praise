...
- 사용자가 승인한 뒤에는 별도 요청이 없는 한 전체 diff를 다시 제시하지 말고, 작은 단위로 파일을 수정한다.
- 전체 unified diff는 사용자가 명시적으로 요청한 경우에만 제시한다.
- `apply_patch`가 실패하면 `perl -0pi`, `ruby File.write`, `python write_text` 같은 직접 파일 덮어쓰기 방식으로 우회하지 않는다.
- `apply_patch`가 실패하면 실제 수정 없이 실패한 파일의 unified diff만 제시하고 사용자 판단을 기다린다.
- 문서 작업도 동일하게 적용한다.
