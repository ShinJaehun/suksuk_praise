module CouponDraw
  class Issue
    ...
    def self.call(classroom:, basis:, mode:, issued_by:, target_user_id: nil)
      ...
      # -> 발급 후보는 personal(bucket=personal)만 사용 (library는 adopt 전용)
      personal_scope = CouponTemplate.where(
      ...
      # personal이 비었거나(0개) / weight 합이 0이면(nil) → 안내 메시지로 막기
      raise NoActiveTemplateError unless template
