module CouponDraw
  class Issue
    ...
    def self.call(classroom:, basis:, mode:, issued_by:, target_user_id: nil)
      ...
      # 2) 정책 검증: 각 기간 top 모드는 해당 기간 칭찬왕(동률 포함)이어야 함
      if top_mode?(basis, mode)
        result = ComplimentKings::Pick.call(classroom: classroom, period: basis, now: now)
        raise NoCandidateError.new("coupons.draw.no_candidate") if result.compliment_count.zero?
        raise NotComplimentKingToday.new("coupons.draw.not_today_king", http_status: :forbidden) unless result.winners.any? { |winner| winner.id == user.id }
      end

      # 3) 기간 시작 계산 & 중복 가드(daily/weekly/monthly period별 1회)
      period_start = UserCoupon.period_start_for(basis, now: now)
      if period_basis?(basis)
        if UserCoupon.period_duplicate_of(
          ...
          raise DuplicatePeriodError.new(duplicate_error_key_for(basis), http_status: :conflict)
    ...
    def self.normalize_basis(basis)
      b = basis.to_s.strip
      case b
      when "manual", "daily", "weekly", "monthly" then b
      else "daily"
      end
    end
    ...
    def self.top_mode?(basis, mode)
      %w[daily weekly monthly].include?(basis) && mode == "#{basis}_top"
    end

    def self.period_basis?(basis)
      %w[daily weekly monthly].include?(basis)
    end

    def self.duplicate_error_key_for(basis)
      case basis
      when "weekly" then "coupons.draw.already_issued_this_week"
      when "monthly" then "coupons.draw.already_issued_this_month"
      else "coupons.draw.already_issued_today"
      end
    end
