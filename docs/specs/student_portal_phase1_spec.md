...
학생 쿠폰 사용 요청은 현재 구현 완료 정책이 아니라 `docs/planning/backlog.md`의 후속 작업 후보다.
구현 전에는 현재 coupon 흐름, 관련 policy, request spec, notification 설계 여부를 다시 확인한다.
...
- 요청 상태, notification 연결 방식, 중복 요청 규칙은 별도 구현 단계에서 구체화하고 테스트한다.
...
상태:
- 현재 구현 완료 범위가 아니라 backlog 후보로 둔다.
- 구현 전 `docs/planning/backlog.md`, 현재 coupon 코드, 관련 spec을 다시 확인한다.
...
이 기능은 현재 구현 상태 문서가 아니라 `docs/planning/backlog.md`에서 관리한다.
