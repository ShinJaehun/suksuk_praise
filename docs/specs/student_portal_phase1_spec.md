...
7. 학생 쿠폰 사용 요청 흐름 도입
8. 교사↔학생 메시지 기능을 수용할 수 있는 구조 정리
...
아바타 표시는 기존 `User#avatar` Active Storage 첨부와
`default_avatar_index` fallback 규칙을 유지한다.

다만 이번 단계에서는 학생 self-service 계정 수정 화면에
아바타 업로드/교체 UI를 노출하지 않는다.
...
- 이번 단계에서는 이미지 크롭 UI, 직접 업로드 최적화, 실제 S3 운영 설정, self-service 업로드 변경 UI까지 포함하지 않는다.

### D. 학생 쿠폰 사용 요청 원칙

이번 단계에서는 학생이 self page에서 쿠폰을 직접 사용 처리하지 않고,
교사/관리자에게 `쿠폰 사용 요청`을 보낼 수 있는 방향을 기준으로 구조를 정리한다.

원칙:

- 학생은 자기 쿠폰에 대해서만 요청할 수 있어야 한다.
- 교사/관리자는 교실 맥락의 학생 상세에서 해당 요청을 확인하고 처리할 수 있어야 한다.
- 학생 self page와 교사/관리자용 학생 상세는 같은 쿠폰 정보를 보더라도 서로 다른 액션을 수용해야 한다.
- 요청 상태, notification 연결 방식, 중복 요청 규칙은 구현 단계에서 구체화하고 테스트한다.

### E. 교사↔학생 메시지 기능 원칙

이번 단계에서는 교사↔학생 메시지 기능의 세부 interaction 을 확정하기보다,
향후 기능을 수용할 수 있도록 학생 self page와 교사/관리자용 학생 상세의 역할을 분리하는 방향을 우선한다.

원칙:

- 메시지 기능은 학생 self page와 교사/관리자용 학생 상세가 서로 다른 목적의 UI를 갖는다는 전제를 따른다.
- 초기 구현은 특정 학생 대상 메시지를 우선 검토하되, 교실 단위 전달 가능성도 열어 둔다.
- 수신 범위(1:1, 1:N, 전체), 답장 정책, 읽음 상태, notification 방식은 구현 직전에 별도 확정한다.
- 따라서 메시지 기능이 추가되는 시점에는 `users/show` 와 `classroom_students/show` 의 view 분리를 우선 검토한다.
...
목표:
- 학생 본인이 자기 이름을 수정 가능하도록 구현

원칙:
- Rails way 우선
- 가능하면 Devise 기본 edit 등록정보 흐름을 최대한 활용
- 학생 본인 정보 수정은 이름 정리를 이번 단계에서 우선 구현한다.
...
목표:
- 학생 self page와 교사/관리자용 학생 상세가 서로 다른 쿠폰 액션을 수용할 수 있도록 구조를 정리한다.

검토 포인트:
- 요청 상태 모델링
- notification 연결 방식
- 중복 요청/이미 처리된 쿠폰 가드
- 학생용 요청 버튼과 교사용 처리 액션 분리

### 8단계. 교사↔학생 메시지

목표:
- 학생 self page와 교사/관리자용 학생 상세가 메시지 기능을 수용할 수 있도록 구조를 정리한다.
