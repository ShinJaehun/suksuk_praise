...
- 정확한 컬럼, validation과 겹치는 기간의 처리 규칙은 모델 구현 직전에 확정한다.
...
운영일 판단을 view나 controller에 분산하지 않고 한 도메인 경계에서 계산한다.
...
정확한 class/module 이름과 경계값 처리 방식은 구현 시 현재 naming과 테스트 구조를 확인해 정한다.
