  ...
  describe 'GET /dashboard' do
    ...
    it "shows the student's weekday activity for the active session classroom" do
      ...
      travel_to Time.zone.local(2026, 4, 8, 10, 0, 0) do
        ...
        create(:compliment, classroom: other_classroom, receiver: student,
                            given_at: Time.zone.local(2026, 4, 7, 10, 0, 0))
        create(:compliment, classroom: classroom, receiver: other_student,
                            given_at: Time.zone.local(2026, 4, 7, 10, 0, 0))
      ...
      expect(graph.css('[data-y-axis-tick]').map do |tick|
        tick['data-y-axis-tick']
      end).to contain_exactly('0', '2', '4', '6', '8')
      ...
      marker_position = lambda do |node|
        node['transform'].scan(/-?\d+(?:\.\d+)?/).map(&:to_f)
      end
      issued_marker_positions = issued_markers.map { |node| marker_position.call(node) }
      used_marker_positions = used_markers.map { |node| marker_position.call(node) }
      issued_bottom_position = marker_position.call(issued_bottom_marker)
      used_bottom_position = marker_position.call(used_bottom_marker)
      tuesday_issued_bottom_position = marker_position.call(tuesday_issued_bottom_marker)
      ...
      expect(graph.at_css('symbol#issued-coupon-icon')).to be_present
      expect(graph.at_css('symbol#used-coupon-icon')).to be_present
      expect(issued_markers.map { |node| node.at_css('use')['href'] }).to all(eq('#issued-coupon-icon'))
      expect(used_markers.map { |node| node.at_css('use')['href'] }).to all(eq('#used-coupon-icon'))
      expect(issued_marker_positions.map(&:first)).to all(be < praise_point_x)
      expect(used_marker_positions.map(&:first)).to all(be > praise_point_x)
      expect(issued_marker_positions.map(&:first).uniq.size).to eq(1)
      expect(used_marker_positions.map(&:first).uniq.size).to eq(1)
      expect(issued_marker_positions.map(&:last).uniq.size).to eq(3)
      expect(used_marker_positions.map(&:last).uniq.size).to eq(2)
      expect(issued_bottom_position.last).to be < zero_y
      expect(used_bottom_position.last).to be < zero_y
      expect(issued_bottom_position.last).to eq(used_bottom_position.last)
      expect(issued_bottom_position.last).to eq(tuesday_issued_bottom_position.last)
      expect(issued_bottom_position.last).not_to eq(praise_point_y)
    ...
    it 'shows an empty state when the student has no weekday activity' do
      ...
      expect(document.css('[data-graph-date]').map do |date|
        date['data-graph-date']
      end).to contain_exactly('4/6', '4/7', '4/8', '4/9', '4/10')
