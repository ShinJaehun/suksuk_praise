...
RSpec.describe 'Compliment templates', type: :request do
  ...
  it 'creates a preset owned by current_user and ignores submitted owner fields' do
    ...
    end.to change { teacher.compliment_presets.active.count }.by(1)
  ...
  it 'keeps historical compliment snapshots after preset deactivation' do
    ...
    create(:compliment, classroom: classroom, giver: teacher, receiver: student, compliment_preset: preset,
                        reason: preset.title)
    sign_in teacher

    delete compliment_template_path(preset)
    get compliment_events_path(kind: 'custom', period: 'all_time')
