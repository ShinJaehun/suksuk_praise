...
RSpec.describe 'Admin teacher school memberships', type: :request do
  ...
  it 'keeps a failed update and its selected school in the modal' do
    ...
    expect(response.body).to include('turbo-stream action="replace" target="modal"')
    expect(response.body.scan('<turbo-frame id="modal"').size).to eq(1)
    expect(response.body).to match(%r{<option selected="selected" value="#{other_school.id}">#{other_school.name}</option>})
  ...
  it 'rejects a different-school classroom without changing existing memberships' do
    existing_classroom = create(:classroom, school: school, grade: 3)
    other_classroom = create(:classroom, school: other_school, grade: 4)
    create(:school_membership, user: teacher, school: school)
    create(:classroom_membership, user: teacher, classroom: existing_classroom, role: 'teacher')
    sign_in admin
    ...
    expect(teacher.reload.school).to eq(school)
    expect(teacher.classroom_memberships.teacher.pluck(:classroom_id))
      .to contain_exactly(existing_classroom.id)
    expect(teacher.classroom_memberships.teacher.exists?(classroom: other_classroom)).to eq(false)
  end
